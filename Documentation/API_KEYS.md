...
```text
Jellyfin Dashboard → Media Tagging Manager → Main Settings → API Settings
```
...
Watchmode is optional fallback provider coverage. The plugin consults it only
when TMDb does not return provider availability for a title.
...
To rotate a credential, replace its value in the plugin settings and save. To
stop using a source, clear its key, save, and restart Jellyfin if you changed
