...
| More Like This: secondary tab, scoped save, load, sync, record-only removal, relationship overview, poster cache | Authorized custom settings/data routes and plugin-owned `DataFolderPath` JSON/cache files. Dedicated relationship work uses the same public `IScheduledTask`/`ITaskManager` path as the plugin's other long-running Dashboard actions. This data is intentionally separate from Jellyfin metadata and does not use a private Jellyfin recommendation API. | Supported Jellyfin integration; TMDb result/caching/rendering behavior requires live checks. |
