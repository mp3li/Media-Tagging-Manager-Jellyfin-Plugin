...
| More Like This: secondary tab, scoped save, relationship overview, poster cache | Authorized custom settings/data routes and plugin-owned `DataFolderPath` JSON/cache files. This data is intentionally separate from Jellyfin metadata and does not use a private Jellyfin recommendation API. | Supported Jellyfin integration; TMDb result/caching/rendering behavior requires live checks. |
