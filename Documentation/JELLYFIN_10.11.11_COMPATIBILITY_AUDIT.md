...
| Provider/Network and Genre/Keyword Library Overviews: filters, expand/collapse, manual edits, save changes | Selected-library `GetItemList` queries and native writes; each overview edits only its displayed tag types and preserves the other plugin-managed tag values. | Supported; rendering and edit behavior require live checks. |
