...
| More Like This: secondary tab, scoped save, load, update, record-only removal, relationship overview, poster cache | Authorized custom settings/data routes and plugin-owned `DataFolderPath` JSON/cache files. Explicit Load and Update routes queue dedicated relationship work through the same public `IScheduledTask`/`ITaskManager` path as the plugin's other long-running Dashboard actions. This data is intentionally separate from Jellyfin metadata and does not use a private Jellyfin recommendation API. | Supported Jellyfin integration; TMDb result/caching/rendering behavior requires live checks. |
| Production Companies and Countries: secondary tab, scoped save, country picker, native metadata update, logo cache, cleanup, overview, direct edit | TMDb title details and configuration-country data feed native `BaseItem.Studios` and `BaseItem.ProductionLocations`; every changed item uses `ILibraryManager.UpdateItemAsync(..., MetadataEdit, ...)`. Plugin-owned provenance remains in its data folder only to constrain cleanup. | Supported public 10.11.11 metadata fields and update path; live checks required for NFO representation, metadata preservation, and cleanup ownership. |
