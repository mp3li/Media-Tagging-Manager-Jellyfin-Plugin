...
| Browser page behavior | The embedded page calls only this plugin's custom authorized endpoints and uses Jellyfin dashboard page primitives already available to embedded pages. It no longer depends on the dashboard's internal `Library/VirtualFolders` or plugin-configuration endpoint. | The custom-controller approach is documented by the official plugin template. The exact web-client rendering still requires live validation against the installed 10.11.11 web build. | Needs live check |
