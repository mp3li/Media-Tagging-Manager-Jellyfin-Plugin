...
The repository now has a real, checksum-backed test manifest for `0.1.0.8-test`. It points to an actual test ZIP and contains no API keys. A stable manifest entry will replace this test entry only after real Jellyfin-server testing is complete. The complete source-level compatibility review is in [the Jellyfin 10.11.11 audit](Documentation/JELLYFIN_10.11.11_COMPATIBILITY_AUDIT.md).

## Test Prerelease Catalog

`0.1.0.8-test` is a public catalog-install test build, **not** a stable release. It exists so the real Jellyfin installation flow can be tested before the first stable package is published.
